# 리퀴파이 기능 구현
# 영상의 일부분을 액체처럼 흐물거리게 하여 모양을 바꾸는 효과

import cv2
import numpy as np

win_title = "Liquify"   # 창 이름
half = 50               # 관심 영역의 절반 크기
isDragging = False      # 드래그 영역 플래그
...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        if not isDragging:
            ...
            cv2.imshow(win_title, img_draw)     # 녹색 사각형으로 영역 표시
